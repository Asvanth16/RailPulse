...
export const alertService = {
  async create(
    ...
    switch (data.alertType) {
      ...
      case "CANCELLATION":
        if (!data.trainNumber) {
          throw new BadRequestError(
            "Train number is required for this alert type.",
          );
        }
        break;
      ...
      case "ARRIVAL_REMINDER":
        if (!data.journeyId && !data.trainNumber) {
          throw new BadRequestError(
            "Journey ID or train number is required.",
          );
        }
        break;
    }
    ...
    const duplicate =
      await alertRepository.findDuplicate(
        userId,
        data,
      );
  ...
  async findAll(
    ...
    const alerts =
      await alertRepository.findByUserId(userId);
  ...
  async findById(
    ...
    if (alert.userId !== userId) {
      throw new ForbiddenError(
        "You cannot access another user's alert.",
      );
    }
  ...
  async update(
    ...
    if (alert.userId !== userId) {
      throw new ForbiddenError(
        "You cannot update another user's alert.",
      );
    }
  ...
  async delete(
    ...
    if (alert.userId !== userId) {
      throw new ForbiddenError(
        "You cannot delete another user's alert.",
      );
    }
